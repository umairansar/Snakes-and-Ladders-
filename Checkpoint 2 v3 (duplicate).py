    ...
    def gameMaster(self, myPlayer, sock):
        ...
        if winner == False:
            ...
            #for normal offline play                        
                ...
                if totalPoints == 18:
                    totalPoints = 0
                    sendMessage(self.mySocket, "mansar", str(totalPoints))
                else:
                    sendMessage(self.mySocket, "mansar", str(totalPoints))
...
def resumeEvent():
    try:
        ...
        core(int(TotalPlayers), temp, main_root, int(whichPlayer), Resume)
...
def checkReply(socket):
    ...
    if ShowMessages(socket):
        if TheMsg(socket) == "okay":
            ...
            global net
            net = socket
            print("ok wala msg se window khuli")
            p2OnlineEvent() 
...
def initializeButtons():
    ...
    rules = Button(main_canvas, text="Rules", width=20, bg = "#E2EBEF",  font="Helvetica 22")
